...
This script uploads all airline BMP files to a specific Zipline folder.
One-time batch upload for ESP32 AirTracker project.

Usage:
    python3 upload_airline_logos_zipline.py --folder "/path/to/airline/bmps" --zipline-url "https://zip.spacegeese.com" --folder-id "cmfw4ypuv000201mvm2ltn1t5"
    python3 upload_airline_logos_zipline.py --help
...
def find_bmp_files(folder_path: Path) -> List[Path]:
...
def upload_file_to_zipline(file_path: Path, zipline_url: str, token: str, folder_id: Optional[str] = None) -> Tuple[bool, str, Optional[str]]:
    ...
    try:
        ...
        files = {
            'file': (file_path.name, open(file_path, 'rb'), 'image/bmp')
        }
...
def batch_upload_bmps(folder_path: Path, zipline_url: str, token: str, zipline_folder_id: Optional[str] = None, delay: float = 0.5) -> Tuple[int, int]:
    """
    Upload all BMP files in folder to Zipline.
    ...
    print(f"🔄 Found {len(bmp_files)} BMP files to upload")
    ...
    for i, file_path in enumerate(bmp_files, 1):
        print(f"[{i:4d}/{len(bmp_files)}] Uploading: {file_path.name}")
        ...
        # Rate limiting delay
        if i < len(bmp_files):  # Don't delay after the last file
            time.sleep(delay)

        print()

    return successful, len(bmp_files)

def main():
    """Main CLI interface."""
    parser = argparse.ArgumentParser(
        description="Upload airline BMP files to Zipline",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 upload_airline_logos_zipline.py --folder "/path/to/bmps"
  python3 upload_airline_logos_zipline.py --folder "/path/to/bmps" --delay 1.0
  python3 upload_airline_logos_zipline.py --folder "/path/to/bmps" --token "your_token"
...
Environment Variables:
  ...
  ZIPLINE_FOLDER_ID - Target folder ID (optional)
    ...
    parser.add_argument('--folder', required=True,
                       help='Folder containing BMP files to upload')
    ...
    parser.add_argument('--token',
                       default=os.getenv('ZIPLINE_TOKEN', 'MTc1ODYwNzc3MDUzMw==.'),
                       help='Zipline authentication token (or set ZIPLINE_TOKEN env var)')
    parser.add_argument('--zipline-folder-id',
                       default='cmfw4ypuv000201mvm2ltn1t5',
                       help='Zipline folder ID to upload to (default: cmfw4ypuv000201mvm2ltn1t5)')
    ...
    if args.dry_run:
        ...
        try:
            ...
            for i, file_path in enumerate(bmp_files[:10], 1):  # Show first 10
                print(f"  {i:3d}. {file_path.name}")

            if len(bmp_files) > 10:
                print(f"  ... and {len(bmp_files) - 10} more files")
    ...
    # Perform upload
    print("🚀 Zipline Airline Logo Uploader")
    print("=" * 50)

    try:
        successful, total = batch_upload_bmps(
            folder_path,
            args.zipline_url,
            args.token,
        ...
        if successful > 0:
            print(f"✅ Successfully uploaded {successful} airline logos to Zipline!")
